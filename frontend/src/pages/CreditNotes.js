...
const CreditNotes = () => {
  ...
  const fetchCreditNotes = useCallback(async () => {
    try {
      ...
      toast.error('Error fetching credit notes: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  ...
  const getDocumentStatus = (creditNote) => {
    if (!creditNote) return 'new';
    // Check documentStatus first
    if (creditNote.documentStatus === 'downloaded') return 'downloaded';
    if (creditNote.documentStatus === 'viewed') return 'viewed';
    if (creditNote.documentStatus === 'queried') return 'queried';
    if (creditNote.documentStatus === 'review') return 'review';
    // If documentStatus is 'ready' and hasn't been viewed, it's "Ready (New)"
    if (creditNote.documentStatus === 'ready' && !creditNote.viewedAt) return 'ready_new';
    // Fallback to timestamp-based status
    if (creditNote.downloadedAt) return 'downloaded';
    if (creditNote.viewedAt) return 'viewed';
  ...
  // Selection handlers
  const handleSelectCreditNote = (creditNoteId) => {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedCreditNotes(creditNotes.map(cn => cn.id));
    } else {
      setSelectedCreditNotes([]);
    }
  };

  const isAllSelected = creditNotes.length > 0 && selectedCreditNotes.length === creditNotes.length;

  // Download handlers
  const handleDownloadCreditNote = async (creditNoteId) => {
    try {
      // Mark as viewed first (fire and forget)
      api.post(`/api/credit-notes/${creditNoteId}/view`).catch(err => console.warn('Failed to mark as viewed:', err));
      
      // Get the download URL
      const baseUrl = process.env.REACT_APP_API_URL || '';
      const token = localStorage.getItem('token');
      const url = `${baseUrl}/api/credit-notes/${creditNoteId}/download`;
      ...
      setTimeout(() => fetchCreditNotes(), 500);
      toast.success('Credit note downloaded');
    } catch (error) {
      console.error('Error downloading credit note:', error);
      toast.error('Error downloading credit note: ' + (error.message || 'Unknown error'));
    }
  };

  const handleBulkDownload = async () => {
    if (selectedCreditNotes.length === 0) {
      toast.info('Please select at least one credit note to download');
      return;
    }

    try {
      const token = localStorage.getItem('token');
      const url = `${process.env.REACT_APP_API_URL || '/api'}/credit-notes/bulk-download`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ creditNoteIds: selectedCreditNotes })
      });
      ...
      const link = document.createElement('a');
      link.href = blobUrl;
      link.setAttribute('download', `credit-notes-${Date.now()}.zip`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(blobUrl);

      toast.success(`Downloaded ${selectedCreditNotes.length} credit note(s)`);
      setSelectedCreditNotes([]);
      fetchCreditNotes();
    } catch (error) {
  ...
  const getPONumber = (creditNote) => {
    return creditNote.metadata?.parsedData?.customerPO || creditNote.metadata?.parsedData?.poNumber || '-';
  };
  ...
  const handleDeleteCreditNote = async () => {
    ...
    try {
      setDeleting(true);
      await api.delete(`/api/credit-notes/${creditNoteToDelete.id}`, {
      ...
      toast.error('Error deleting credit note: ' + (error.response?.data?.message || error.message));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleBulkDelete = async () => {
    if (selectedCreditNotes.length === 0) {
      toast.info('Please select at least one credit note to delete');
      return;
    }
    ...
    try {
      ...
      const response = await api.post('/api/credit-notes/bulk-delete', {
      ...
      if (failed > 0) {
        toast.warning(`Deleted ${deleted} credit note(s), ${failed} failed`);
      } else {
        toast.success(`Successfully deleted ${deleted} credit note(s)`);
      }
      ...
      toast.error('Error deleting credit notes: ' + (error.response?.data?.message || error.message));
    } finally {
      setBulkDeleting(false);
    }
  };

  // Credit note import functions
  ...
  const handleImportCreditNotes = async () => {
    ...
    try {
      ...
      const response = await api.post('/api/credit-notes/import', formData, {
        headers: {
  ...
  return (
    <div className="page">
                      ...
                      return (
                        <tr key={creditNote.id}>
                          ...
                          {queriesEnabled && (
                            <td>
                              {(() => {
                                const queryInfo = creditNote.queryInfo;
                          ...
                          {settings?.documentRetentionPeriod && (
                            <td>
                              <DocumentRetentionTimer
                                expiryDate={creditNote.retentionExpiryDate}
                                startDate={creditNote.retentionStartDate}
                          ...
                          <td>
                            <div className="btn-list">
                              ...
                              <button 
                                ...
                                onClick={() => handleDownloadCreditNote(creditNote.id)}
                                title="Download"
                              >
                                Download
                              </button>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Importing Credit Notes</h5>
                            ...
                            <tr key={idx} className={result.success ? '' : 'table-danger'}>
                              ...
                              <td>
                                {result.creditNoteNumber ? (
                                  <span className="font-weight-medium">{result.creditNoteNumber}</span>
                                ) : (
                                  <span className="text-muted">-</span>
                                )}
                              </td>
      ...
      {showBulkDeleteModal && selectedCreditNotes.length > 0 && (
        <div className="modal modal-blur fade show" style={{ display: 'block' }} tabIndex="-1">
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Delete Multiple Credit Notes</h5>
                <button 
                  type="button" 
                  className="btn-close" 
                  onClick={() => {
              ...
              <div className="modal-body">
                <div className="mb-3">
                  <p className="text-danger">
                    <strong>Warning:</strong> This action cannot be undone. {selectedCreditNotes.length} credit note(s) and their associated files will be permanently deleted.
                  </p>
                  <p>
                    You are about to delete the following credit notes:
                  </p>
                  <ul className="list-unstyled">
                    {creditNotes
                      .filter(cn => selectedCreditNotes.includes(cn.id))
                      .slice(0, 10)
                      .map(cn => (
                        <li key={cn.id} className="text-muted">
                          • {cn.creditNoteNumber} - {cn.company?.name || 'Unknown Company'}
                        </li>
                      ))}
                    {selectedCreditNotes.length > 10 && (
                      <li className="text-muted">... and {selectedCreditNotes.length - 10} more</li>
                    )}
                  </ul>
                </div>
                <div className="mb-3">
                  ...
                  <textarea
                    className="form-control"
                    rows="4"
                    placeholder="Please provide a reason for deleting these credit notes (required for accountability)..."
                  ...
                  <small className="form-hint">
                    This reason will be recorded in the system for each deleted credit note for accountability purposes.
                  </small>
                </div>
              </div>
              <div className="modal-footer">
                ...
                >
                  {bulkDeleting ? 'Deleting...' : `Delete ${selectedCreditNotes.length} Credit Note(s)`}
                </button>
              </div>
            </div>
          </div>
      ...
      {showDeleteModal && creditNoteToDelete && (
        <div className="modal modal-blur fade show" style={{ display: 'block' }} tabIndex="-1">
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Delete Credit Note</h5>
                <button 
                  type="button" 
                  className="btn-close" 
                  onClick={() => {
              ...
              <div className="modal-body">
                <div className="mb-3">
                  <p className="text-danger">
                    <strong>Warning:</strong> This action cannot be undone. The credit note and its associated file will be permanently deleted.
                  </p>
                  <p>
                    You are about to delete credit note <strong>{creditNoteToDelete.creditNoteNumber}</strong> for <strong>{creditNoteToDelete.company?.name || 'Unknown Company'}</strong>.
                  </p>
                </div>
                <div className="mb-3">
                  <label className="form-label required">Reason for Deletion</label>
                  <textarea
                    className="form-control"
                    rows="4"
                    placeholder="Please provide a reason for deleting this credit note (required for accountability)..."
                    value={deleteReason}
