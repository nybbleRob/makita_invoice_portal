...
const CreditNoteView = () => {
  ...
  const fetchCreditNote = async () => {
    try {
      ...
      // Credit note is automatically marked as viewed by the backend
  ...
  const handleDownload = async () => {
    try {
      ...
      // Refresh credit note to update status
      fetchCreditNote();
  ...
  const fetchQueryHistory = async () => {
    try {
      ...
      const response = await api.get(`/api/document-queries/credit_note/${id}`);
  ...
  const handleSendQuery = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/document-queries/credit_note/${id}`, {
  ...
  const handleReplyToQuery = async () => {
    ...
    try {
      setSendingMessage(true);
      const response = await api.post(`/api/document-queries/credit_note/${id}/reply`, {
        message: newMessage.trim(),
        replyTo: replyingTo || null
      });
  ...
  const handleResolveQuery = async () => {
    ...
    try {
      setResolving(true);
      const response = await api.post(`/api/document-queries/credit_note/${id}/resolve`, {
      ...
      <div className="page-header">
        <div className="container-xl">
          <div className="row g-2 align-items-center">
            <div className="col">
              <h2 className="page-title">Credit Note Details</h2>
              <div className="text-muted mt-1">Credit Note #{creditNote.creditNoteNumber}</div>
            </div>
            <div className="col-auto ms-auto">
              ...
              >
                {downloading ? 'Downloading...' : 'Download Credit Note'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="page-body">
        <div className="container-xl">
          <div className="row">
              ...
              <div className="card">
                ...
                <div className="card-body p-0">
                  {loadingPdf ? (
                    ...
                    <div className="text-center py-5">
                      <p className="text-muted">No PDF file available for this credit note</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Credit Note Information - Right Side (33%) */}
            <div className="col-lg-4">
              <div className="card">
                <div className="card-header d-flex justify-content-between align-items-center">
                  <h3 className="card-title">Credit Note Information</h3>
                  {creditNote && settings?.documentRetentionPeriod && (
                ...
                <div className="card-body">
                  <div className="list-group list-group-flush">
                    <div className="list-group-item px-0 py-2">
                      <div className="d-flex flex-row align-items-center justify-content-between">
                        ...
                        <div className="text-end" style={{ flex: 1 }}>
                          <span className="badge bg-primary-lt">Credit Note</span>
                        </div>
                      </div>
                    </div>
                    <div className="list-group-item px-0 py-2">
                      <div className="d-flex flex-row align-items-center justify-content-between">
                        ...
                        <div className="fw-semibold text-end" style={{ flex: 1 }}>
                          {creditNote.company ? creditNote.company.name : '-'}
                        </div>
                      </div>
                    </div>
                    <div className="list-group-item px-0 py-2">
                      <div className="d-flex flex-row align-items-center justify-content-between">
                        ...
                        <div className="fw-semibold text-end" style={{ flex: 1 }}>
                          {creditNote.company?.referenceNo || '-'}
                        </div>
                      </div>
                    </div>
                    <div className="list-group-item px-0 py-2">
                    ...
                    {creditNote?.metadata?.parsedData && availableFields.length > 0 && (() => {
                      const customFields = availableFields.filter(f => f.isCustom);
                      const customFieldValues = customFields
                        .filter(f => creditNote.metadata.parsedData[f.fieldName] !== undefined && creditNote.metadata.parsedData[f.fieldName] !== null && creditNote.metadata.parsedData[f.fieldName] !== '')
                        .map(f => ({
                          field: f,
                          value: creditNote.metadata.parsedData[f.fieldName]
                        }));
              ...
              {/* Query Section - Below Credit Note Information (only if enabled) */}
              {queriesEnabled && (
                      ...
                      <div className="border-top pt-3">
                        ...
                        <div className="mb-2">
                          <label className="form-label">
                            {queryMessages.length === 0 
                              ? (currentUser?.role === 'external_user' ? 'Send a query about this credit note' : 'Send a query to customer')
                          ...
                          <textarea
                            ...
                            placeholder={queryMessages.length === 0
                              ? (currentUser?.role === 'external_user' 
                                ? 'Enter your question or concern about this credit note...'
